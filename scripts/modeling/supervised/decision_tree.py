...
def apply_decision_tree(self, cv, supplied, test_size , params):
    ...
    if cv is not None:
        # Cross-validation
        model, score_mean, scores_str = cross_val(function=DecisionTreeClassifier, cv=cv, X=X, y=y ,params=params)
        cv_used = True
        model.fit(X, y)
    ...
    output_text = "==== Decision Tree Results ====\n\n"
    ...
    if not cv_used:
        ...
        output_text += f"Number of test samples: {X_test.shape[0]}\n"
        ...
        matrix = get_confusion_matrix(model, X, y)
    ...
    return output_text, model, matrix, new_df
